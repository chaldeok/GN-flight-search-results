...
import json from "../jsons/flights.json";
...
const filterWithTransfer = flights => {
  return flights.filter(
    f =>
      ...
      f.flight.legs[f.flight.legs.length - 1].segments.length > 1
  );
};
...
const filterAirlines = (flights, airlines) => {
  ...
  airlines.forEach(a => {
    result = [
      ...result,
      ...flights.filter(f => f.flight.carrier.airlineCode === a.airlineCode)
    ];
  });
  return result;
};
...
const filterPriceRange = (flights, state) => {
  return flights.filter(
    f =>
      +f.flight.price.total.amount >= state.priceMin &&
      +f.flight.price.total.amount <= state.priceMax
  );
};
...
export default new Vuex.Store({
  ...
  getters: {
    flights(state, getters) {
      ...
      if (state.sort === "duration") { //timeDuration
      ...
      if (
        state.airlines.some(a => a.checked === true) &&
        state.airlines.some(a => a.checked === false)
      ) {
  ...
  mutations: {
    ...
    setPriceMax(state) {
      state.priceMax = Math.max.apply(
        null,
        state.flights.map(f => +f.flight.price.total.amount)
      );
    },
  ...
  actions: {
    fetchFlights({ commit }) {
      const flights = json.result.flights;
